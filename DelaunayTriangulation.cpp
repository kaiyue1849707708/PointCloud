...
DelaunayTriangulation::DelaunayTriangulation()
{
    ...
    _ProjectedDots = new std::vector<Vector3D*>();
    _Mesh = new std::vector<Triangle*>();
...
vector<tuple<int, int, int>*> DelaunayTriangulation::GetTriangulationResult(vector<Vector3D*> &dots)
{
    ...
    // generate output
    vector<tuple<int, int, int>*> mesh = std::vector<std::tuple<int, int, int>*>();
...
std::string DelaunayTriangulation::GetStatistics()
{
    ...
    return "\nTriangle count: "
        + regex_replace(std::to_string(_Mesh->size()), regex, "$&,")
